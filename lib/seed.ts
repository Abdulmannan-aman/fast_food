...
interface MenuItem {
    name: string;
    description: string;
    image_url: string; // already uploaded, use directly
    price: number;
    rating: number;
    calories: number;
    protein: number;
...
async function seed(): Promise<void> {
    ...
    // 4. Create Menu Items (use existing image URLs)
    const menuMap: Record<string, string> = {};
    for (const item of data.menu) {
        const doc = await databases.createDocument(
            appwriteConfig.databaseId,
            ...
            ID.unique(),
            {
                name: item.name,
                description: item.description,
                image_url: item.image_url, // use directly (no upload)
                price: item.price,
                rating: item.rating,
                calories: item.calories,
                protein: item.protein,
    ...
    console.log("✅ Database seeding complete (images skipped).");
}
